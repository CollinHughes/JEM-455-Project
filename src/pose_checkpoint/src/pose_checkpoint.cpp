...
geometry_msgs::Vector3 pose_error;
geometry_msgs::Vector3 pose_v3;
std_msgs::UInt16 reset_num;
std_msgs::UInt16 last_reset;
int new_data = 0;
int pose_number = 0;

void error_in_fun(const geometry_msgs::Vector3& E) {
	pose_error = E;
	new_data = 1;
}

void reset_in_fun(const std_msgs::UInt16& R) {
	reset_num.data = R.data;
}
...
int main(int argc, char **argv) {
    ...
    // Collin added setup:
    ros::Subscriber error_in = n.subscribe("/error", 1, error_in_fun);
    ros::Subscriber reset_in = n.subscribe("/reset", 1, reset_in_fun);
    ...
    {
    	if ((new_data == 1) & (pose_error.x == 0.0) & (pose_error.y == 0.0) & (pose_error.z == 0.0)) {
    		pose_v3.x = pose_list[pose_number].x;
    		pose_v3.y = pose_list[pose_number].y;
    		pose_v3.z = pose_list[pose_number].theta;
    		if (pose_number < pose_list.size()) {
    			pose_out.publish(pose_v3);
    			ROS_INFO("Going to: %d: x=%.2f, y=%.2f, theta=%.2f", pose_number, pose_list[pose_number].x, pose_list[pose_number].y, pose_list[pose_number].theta);
    			pose_number++;	
    		}
    	}
    	if ((reset_num.data == 1) & (last_reset.data != 1)) {
    	...
    	last_reset.data = reset_num.data;
    	new_data = 0;
        ros::spinOnce();
    }
}
